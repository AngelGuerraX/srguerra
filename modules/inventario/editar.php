...
// FICHA DEL PRODUCTO + STOCK MULTI-ALMACÉN (SOLO LECTURA) + RASTREADOR
...
// ---------------------------------------------------------
// 3. RASTREADOR: FILTROS (Igual que antes)
// ---------------------------------------------------------
...
foreach($movimientos as $m) {
    ...
    if($m['estado_interno'] == 'En Ruta') $unidades_en_ruta += $m['cantidad_vendida'];
}
    ...
    <div class="col-lg-4">
        ...
        <div class="card-glass p-4">
            <div class="d-flex justify-content-between align-items-center mb-3">
                <h5 class="text-white fw-bold m-0"><i class="fas fa-boxes me-2"></i> Inventario Actual</h5>
            ...
            <div class="vstack gap-2 mb-3">
                ...
                $total_calculado = 0;
                if(empty($stocks)): ?>
                    <div class="text-center text-muted small py-2">No hay almacenes creados.</div>
                <?php else:
                    foreach($stocks as $s): 
                        $total_calculado += $s['cantidad'];
                        
                        // Color condicional para stock
                        $color_stock = 'text-white';
                        if($s['cantidad'] == 0) $color_stock = 'text-muted';
                        if($s['cantidad'] < 0) $color_stock = 'text-danger fw-bold'; 
                    ?>
                    <div class="d-flex justify-content-between align-items-center border-bottom border-secondary border-opacity-25 pb-2">
                        <label class="text-white-50 small m-0"><?php echo $s['nombre']; ?></label>
                        <span class="<?php echo $color_stock; ?> fs-6"><?php echo $s['cantidad']; ?> u.</span>
                    </div>
                    <?php endforeach; 
                endif; ?>
            </div>
            ...
            <div class="d-flex justify-content-between align-items-center pt-2 border-top border-secondary">
                ...
                <span class="text-neon fw-bold fs-4"><?php echo $total_calculado; ?></span>
            </div>
